class MainMenu
  ...
  def show_trains
    ...
    Station.all[choice].list do |train|
  ...
  def change_route
    loop do
      ...
      when 1
        ...
        name = gets.to_i
        unless Route.all[choice - 1].stations.include?(Station.all[name - 1])
          Route.all[choice - 1].add_station(Station.all[name - 1])
        end
        break
  ...
  def manage_wagons
    ...
    when 1
      if Train.all[train_choice - 1].type == :passenger
        ...
        seats = gets.to_i
        Train.all[train_choice - 1].add_wagon PassengerWagon.new(seats)
      else
        print 'Введите объем вагона: '
        volume = gets.to_i
        Train.all[train_choice - 1].add_wagon CargoWagon.new(volume)
      end
    when 2
      Train.all[train_choice - 1].delete_wagon
    when 3
      wagon_choice = choose_wagon(Train.all[train_choice - 1])
      if Train.all[train_choice - 1].type == :passenger
        Train.all[train_choice - 1].wagons[wagon_choice - 1].take_seat
        puts 'место успешно занято'
      else
        Train.all[train_choice - 1].wagons[wagon_choice - 1].take_volume
        puts 'объем успешно загружен'
      end
    end
  end
  ...
  def show_wagons
    ...
    Train.all[choice - 1].list do |wagon|
      if wagon.is_passenger?
        puts "Пассажирский вагон, свободных мест #{wagon.free_seats}, занято #{wagon.seats_taken} мест"
      else
        puts "Грузовой вагон, свободный объем #{wagon.free_volume}, занятый объем #{wagon.volume_taken} "
      end
    end
  end
  ...
  def show_all_trains
    ...
    Train.all.each do |train|
      puts "#{index_train}) #{train.number} тип #{train.type}"
      index_train += 1
    end
  end
